...
#   python3.13-dev    — Python headers required by torch.compile / Triton C++ codegen
#   libsndfile1       — soundfile audio backend
#   libgomp1          — OpenMP (faiss, onnxruntime)
#   g++               — C++ compiler required by torch.inductor to build generated kernels
#   ninja-build       — parallel build tool used by torch.inductor (speeds up compilation)
RUN apt-get update && apt-get install -y --no-install-recommends \
    gpg-agent \
    curl \
